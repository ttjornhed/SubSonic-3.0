...
namespace PetaPoco
{
	...
	public class Database : IDisposable
	{
		...
		public IEnumerable<T> Query<T>(string sql, params object[] args) 
		{
			...
			try
			{
				using (var cmd = CreateCommand(_sharedConnection, sql, args))
				{
					...
					var factory = pd.GetFactory(cmd.CommandText, _sharedConnection.ConnectionString, ForceDateTimesToUtc, 0, r.FieldCount, r, null) as Func<IDataReader, T>;
					using (r)
					{
						while (true)
						{
		...
		public class PocoData
		{
			...
			// This overload for backwards compatibility where no explicit field -> property mapping is being provided
			...
			{
			    return GetFactory(sql, connString, ForceDateTimesToUtc, firstColumn, countColumns, r, null);
			}

			// Create factory function that can convert a IDataReader record into a POCO
			public Delegate GetFactory(string sql, string connString, bool ForceDateTimesToUtc, int firstColumn, int countColumns, IDataReader r, List<string> columnNames)
			{
				// Check cache
					...
					if (type == typeof(object))
					{
						...
						else
						{
							...
							for (int i = firstColumn; i < firstColumn + countColumns; i++)
							{
                                ...
                                string propertyName = r.GetName(i);
